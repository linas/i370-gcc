...
;; -dP -- put the rtl in with the assembly.
;; -dr -- dump very early rtl out of line
;; -dE -- dump rtl late in the game, after register assignments, etc.
;; more -d flgs in toplev.c
...
;
; This is wrong, as written, because that is not how CH works.
; CH actually does a 32-bit compare, AFTER sign-extending the
; second argument. In this case, the =H'0' sign-extends to a
; 32-bit zero, and so this pattern was just comparing the operand
; to a 32-bit zero. Ooops!
; This might have worked, if the compiler had know that it should
; run truncsihi2 first. But it doesn't know this, and so this
; pattern is just borken as written.
;(define_insn "tsthi"
;  [(set (cc0)
   ...
   "
{
  ...
  if (GET_CODE (op1) == REG
	  ...
	  && (unsigned long) INTVAL (XEXP (op1, 1)) < 4096))
    {
      op1 = operands[1];
    }
  else
  ...
  if (GET_CODE (op2) == REG
	  ...
	  && (unsigned long) INTVAL (XEXP (op2, 1)) < 4096))
    {
      op2 = operands[2];
    }
  else
  ...
  if (GET_CODE (operands[3]) == CONST_INT
      && (unsigned long)INTVAL (operands[3]) <= 256)
    {
  ...
  "*
{
  ...
  if (REG_P (operands[0]))
      ...
      if (operands[1] == const0_rtx)
	{
      ...
      if (GET_CODE (operands[1]) == CONST_INT
  	  && (unsigned long) INTVAL (operands[1]) < 4096)
	{
	  CC_STATUS_INIT;
	  mvs_check_page (0, 6, 0);
...
;
; The movstrictqi and movstricthi seem to evoke a compiler bug, detailed
; below. Lets begin with the symptoms: an IC is issued, followed by a
; branch, when there should have been an intervening compare."
;
; Here's a typical example:
;
;    CR r10,r5
;    BE .L8
;    <<<< optimizer removed a CR r10,r5 here and that's OK.
;    BH .L15
;    IC r7,=XL1'00'
;    IC r5,=XL1'08'
;    <<<< optimizer removed a CR r10,r5 here and that's BAD!!!
;    BE .L8
;
; The optimizer that is doing this is in gcc/final.c in final_scan_insn()
; between lines 2155 and 2188. The actual deletion is at line 2183.
; It notices that the two deleted compares have the same form CR r10,r5
; and so it decides it can avoid re-issueing them. For reasons unclear,
; it is not aware that the value in r5 was clobbered by the prior IC r5,
; cause somehow I gues we should have said "clobber operand 0" ???
; How is it that it just didn't know? The +d should have made it clear
; that its both read and written. So ???
;
; The simple solution is to just comment out the whole insn defintion.
; The "good optimization" above is still performed, and the bad one is
; not. Unclear to me what movstrictqi was supposed to be good for,
; anyway. Using IC is screwy for this const, LA seems better.
;
; This failure mode seems identical to that for movstricthi, below.
;
   ...
   "
{
  ...
  if (GET_CODE (op0) == REG
	  ...
	  && (unsigned long) INTVAL (XEXP (op0, 1)) < 4096))
    op0 = operands[0];
  else
  ...
  if (GET_CODE (operands[2]) == CONST_INT
      && (unsigned long) INTVAL (operands[2]) <= 256)
...
; Typically seen in memcpy(to,from,sizeof(from))

(define_insn ""
  [(set (match_operand:BLK 0 "r_or_s_operand" "=dmS")
	(match_operand:BLK 1 "r_or_s_operand" "dmS"))
...
;
; How this works is a bit opaque, so here's a short explanation. In HFP,
;    =XL8'4110...' is floating 1.0
;    =XL8'4180...' is floating 8.0
;    =XL8'4008...' is denormalized floating 8.0 = 1<<3
;    =XL8'4108...' is denormalized floating 16 * 8.0 = 1<<7
;    =XL8'4208...' is denormalized floating 16**2 * 8.0 = 1<<11
;    =XL8'4F08...' is denormalized floating 16**15 * 8.0 = 1<<63
; Adding this to any long float places the radix at bit position 63.
; The low 32 bits is then the SI integer value.
; I have no clue why this uses the denormalized value, instead of the
; normalized =XL8'5080...'. Perhaps the denormalization forces the
; hardware to do something clever with the guard bits?
...
; Convert SI to Long HFP ("Hex Floating Point")
;
; LE/370 mode uses the float field of the TCA.
; Slightly opaque, so here's what happens.
;
; MVC .., =XL4'4E000000' sets the characteristic to 64 + 14
;   so exponent is 14. This places the radix at 16^14 = 2^56
;   which is bit-position 63. Thus, any integer placed in the
;   fraction will be that integer.
; XI ..,128 exclusive or immediate just flips the sign bit
;   which is at bit zero for a 32-bit int, or bit 32 for
;   the denormalized 64-bit float.
; LD f0,.. load double loads 64 bits into fp reg
; SD f0,=XL8'4E00000080000000' subtracts a denormalized
;   2^31 = 2147483648 undoing the damage caused by the earlier XI
;   but also creating a normalized long HFP in the process
;   (because subtract always normalizes).
;
; Net effect is that this is just a weird branch-free trick for setting
; the sign bit and normalizing at the same time, without having to
; count leading zeros in the integer (to normalize it).
  ...
  "*
{
  ...
  return \"MVC	\" CONVLO \"(4,r13),=XL4'4E000000'\;ST	%1,\" CONVHI \"(,r13)\;XI	\" CONVHI \"(r13),128\;LD	%0,\" CONVLO \"(,r13)\;SD	%0,=XL8'4E00000080000000'\";
  ...
  /* 504 and 508 are in the TCA. r12 holds a TCA pointer for LE/370.  */
...
(define_insn ""
  ...
  "((REGNO (operands[1]) == FRAME_POINTER_REGNUM || REGNO (operands[1]) == ARG_POINTER_REGNUM || REGNO (operands[1]) == STACK_POINTER_REGNUM) && (unsigned long) INTVAL (operands[2]) < 4096)"
  "*
{
  check_label_emit ();
...
{
  check_label_emit ();
  if ((unsigned long) INTVAL (operands[2]) < 4096)
    {
      CC_STATUS_INIT;  /* add assumes CC but LA doesn't set CC */
      mvs_check_page (0, 4, 0);
      return \"LA	%0,%c2(,%1)\";
  ...
  "
{
  if (GET_CODE (operands[1]) == CONST_INT
      ...
      if (got_const)
	{
	  ...
	  /* Take the 32-bits of the const and slap it into the
	   * 64-bit scratch reg. The high bits are already 0xff or 0x0*/
	  emit_insn (gen_rtx_SET (VOIDmode,
		    gen_lowpart (SImode, scratch), consty));
#else
	  emit_insn (gen_rtx_SET (VOIDmode,
	  ...
	  /* Do the actual multiply now, using the mult:DI pattern. */
	  emit_insn (gen_rtx_SET (VOIDmode, scratch,
		    gen_rtx_MULT (DImode, scratch, other)));
	}
      else
	{
	    /* Stick first operand into double-wide scratch reg.
	     * I hope it gets sign extended correctly? I don't get it.
	     */
	    emit_insn (gen_rtx_SET (VOIDmode,
		gen_rtx_SUBREG (SImode, scratch, GET_MODE_SIZE (SImode)),
			operands[1]));
	    ...
	    /* Do the actual multiply now, using the mult:DI pattern. */
	    emit_insn (gen_rtx_SET (VOIDmode, scratch,
		gen_rtx_MULT (DImode, scratch, operands[2])));
	}

      /* Move result to operand[0] */
      emit_insn (gen_rtx_SET (VOIDmode, operands[0],
		gen_rtx_SUBREG (SImode, scratch, GET_MODE_SIZE (SImode))));
    }
  DONE;
...
;; This works great, and is much easier than the above define_expand.
;; However, MS and MSR arn insns first introduced in z/Architecture
;; and are not availble on Sytem/370. So, for now, we dont do this.
;;
;;(define_insn "mulsi3"
;;  [(set (match_operand:SI 0 "register_operand" "=d")
  ...
  "*
{
  check_label_emit ();
  mvs_check_page (0, 4, 2);
  /* In z/Archiecture, we could use MHI instead. */
  return \"MH	%0,%H2\";
}"
   [(set_attr "length" "4")]
)

; See mulsi3 comment above as to why operand 2 is constrained to
; "di" rather than "g"
(define_insn ""
  [(set (match_operand:DI 0 "register_operand" "=d")
  ...
  "*
{
  ...
  if (REG_P (operands[0]))
    {
      /* %K2 == sign extend 16-bit operand to 32 bits. */
      if (GET_CODE (operands[2]) == CONST_INT)
        {
          mvs_check_page (0, 4, 4);
          return \"N	%0,%K2\";
  ...
  "*
{
   ...
   * probably because the 64-bit long-long test is emulated ?! */
  CC_STATUS_SET (operands[0], operands[1]);
  mvs_check_page (0, 4, 0);
  if (REG_P (operands[2]))
    return \"SLDA	%0,0(%2)\";
  ...
  "*
{
  ...
  if (REG_P (operands[0]))
    {
      mvs_check_page (0, 2, 0);
      return \"BR	%0 # indirect_jump\";
    }
  if (i370_short_branch(insn) || mvs_check_label (CODE_LABEL_NUMBER (operands[0])))
    {
      mvs_check_page (0, 4, 0);
      return \"B	%0 # indirect_jump\";
    }
  mvs_check_page (0, 2, 4);
  return \"L	14,=A(%l0)\;BR	14 # indirect_jump\";
}"
  [(set_attr "length" "6")]
)
...
;
; DS 0F means "align on fullword (4-byte) boundary"
